...
| `LOG_LEVEL` | `INFO` | Logging level before a pipeline is loaded, and the level a pipeline declaring no `runtime.logging.log_level` runs at. Must be one of `DEBUG`/`INFO`/`WARNING`/`ERROR`/`CRITICAL` |
