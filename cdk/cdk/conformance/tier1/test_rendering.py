...
class TestStageRendering:
    """The stage DDL carries the declared scope and names the stage."""

    def test_stage_ddl_matches_declared_scope(
        self, conformance_target: ConformanceTarget
    ) -> None:
    ...
    def test_stage_ddl_names_the_stage(
        self, conformance_target: ConformanceTarget
    ) -> None:
    ...
    def test_stage_name_fits_the_identifier_budget(
        ...
        dialect, caps = require_stage_rendering(conformance_target)
        long_target = dialect.table_address(
            "conformance_" + "x" * 80, schema="conformance"
        )
...
class TestModeStatements:
    """The one mode statement matches the declared form and semantics."""
...
class TestReplayDeterminism:
    """A retried batch computes the same plan; distinct batches never collide."""
...
class TestRefusals:
    """Needed-but-undeclared or unusable configuration refuses loudly."""
    ...
    def test_catalog_refused_when_undeclared(
        self, conformance_target: ConformanceTarget
    ) -> None:
