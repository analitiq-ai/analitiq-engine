...
Four places where the published contract is wider than the CDK's api path:
the transport a request dispatches through, the path placeholders, the
declared headers, and the query-key map. None is materialized, and no
failure is visible at runtime -- the request goes out, just not the one the
author wrote. Driving the read cannot find them, because there is no
execution to drive; the only way to report them is to read the declaration.
"""
...
def test_no_read_places_a_param_where_the_path_cannot_send_it(
    conformance_target: ConformanceTarget,
) -> None:
    """A path placeholder is never substituted and a declared header never sent."""
    violations = check_api_request_placements(conformance_target)
    if violations:
        pytest.fail(violation_report(violations))
