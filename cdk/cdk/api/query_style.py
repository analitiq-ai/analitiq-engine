...
class QueryStyle:
    """One param's declared wire serialization: ``style`` and ``explode``."""
...
#: defined spelling, so nothing can send it.
...
def declared_query_styles(
    ...
    for key, node in declared_query.items():
        ...
        if isinstance(style, str) and isinstance(explode, bool):
            styles[str(key)] = QueryStyle(style, explode)
    return styles
...
def unserializable_style_problem(
    ...
    A static fact about the document -- the pair is defined or it is not,
    on every connection and for every value -- so it is judged with the
    rest of the request block, before a page is fetched, rather than on
    the first value that happens to be a collection.
    """
    if (style.style, style.explode) in _DEFINED:
        return None
