...
7. [x] Re-run all 5 disk pipelines + cloud sim end-to-end. Completed 2026-05-19.
   - In-process verification (`tests/integration/test_resolved_construction.py::test_pipeline_engine_glue`) passes for all 5 pipelines: `PipelineConfigPrep().create_config()` → `StreamingEngine.from_resolved(resolved)` → `engine._build_context(stream, …)` is clean for every stream.
   - Docker runs: typed-transport refactor (commit below) unblocks all 5 destination startups. End-to-end results against the live remote APIs:
     - 1/5 (`59072576-…` Wise→PG): pipeline runs end-to-end; 68 records extracted, batch round-tripped through gRPC, schema created in PG, records DLQ'd at the Arrow cast because the Wise API returns timestamps as `'YYYY-MM-DD HH:MM:SS'` (no TZ) and the endpoint declares `Timestamp(MICROSECOND, UTC)` — pre-existing data-shape mismatch, not transport.
     - 1/5 (`0f2412d2-…` Wise→sevdesk, api→api): **end-to-end success**. 68 records processed, 0 failed.
     - 2/5 (`e4e87027-…`, `31fbc951-…` MySQL→PG): **end-to-end success.** Initial run surfaced a PyMySQL 1.2 / aiomysql 0.3 / SQLAlchemy 2.x interaction — PyMySQL 1.2 dropped `Connection.ping()`'s `reconnect=True` default to `False`, which makes SA's `_send_false_to_ping` skip the explicit `False` argument, but the aiomysql adapter still requires the positional. Pinned `PyMySQL<1.2` in `docker/requirements.txt`. Both pipelines now process **15,000 records / 0 failed / status=success**.
     - 1/5 (`47fa9036-…` sevdesk→PG): sevdesk returns `text/html` 200s for the API endpoints (placeholder local credentials hitting an authenticated path). Expected with the test secret store.
   - Implication: the engine + transport layer is now functional end-to-end against real remote APIs and gRPC destinations. Remaining failures are content / dep / credential issues that pre-existed and live outside the schema-alignment refactor's scope.
