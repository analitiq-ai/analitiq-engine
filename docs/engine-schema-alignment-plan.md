...
7. [x] Re-run all 5 disk pipelines + cloud sim end-to-end. Completed 2026-05-19.
   ...
   - Docker runs: typed-transport refactor (commit below) unblocks all 5 destination startups. End-to-end results against the live remote APIs:
     ...
     - 2/5 (`e4e87027-…`, `31fbc951-…` MySQL→PG): blocked on a pre-existing `aiomysql 0.2.0 + SQLAlchemy 2.0` compatibility bug — `AsyncAdapt_aiomysql_connection.ping()` signature changed and SQLAlchemy's `pool_pre_ping=True` calls it without the new `reconnect` argument. Pure dep-version issue, unrelated to this refactor.
