...
On the ADBC transport (Snowflake/BigQuery), keyless `insert` dedups via stage + `MERGE` on `_record_hash` (issue #285), but keyed `insert` is still a plain append — at-least-once until #389 aligns that transport on the same anti-join; `upsert` is idempotent on both transports.
