...
- **file / S3** — batch files are content-addressed (the filename carries a hash of the serialized bytes, #319), so a true replay overwrites the same file with the same bytes; there is no batch-level commit ledger, so nothing can misclassify a restart batch as a replay and drop its rows (#306). A same-run restart re-reads the inclusive cursor boundary and writes those rows into a new file — duplicates possible, drops not. Reported as at-least-once.
