...
- **`truncate_insert`** — full refresh: TRUNCATE once per run on the run's first write (issue #307), plain append after that. The engine never resumes a truncate_insert stream from a cursor — a restart re-reads the source from scratch and the fresh process re-truncates on its first batch.
...
- **SQL `truncate_insert`** — the truncate runs once per run (issue #307) and the append phase has no row-identity dedup (deduping a full refresh would collapse legitimate duplicate rows), so a replayed already-committed batch re-inserts its rows. Reported as at-least-once. The restart data-loss case is gone: the engine never resumes a truncate_insert stream from a cursor, so a restart is a fresh full refresh, not a resumed slice.
