...
class TestFromPylistNullabilityEnforcement:
    ...
    A single post-build check in from_pylist covers all paths uniformly; this
    class verifies the coverage is truly global, not family-specific.
    """
    ...
    def test_all_none_non_nullable_raises(self):
        c = self._contract("Int64", nullable=False)
        with pytest.raises(ValueError, match="every source value is None"):
