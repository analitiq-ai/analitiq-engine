...
class TestTransportRetryPolicy:
    """The declaration is the authority on a status's retryability.

    The RetryClient runs *before* the ack verdict, so a declared-fatal
    status left in its set would be hammered anyway — burning the
    destination ACK deadline before the fatal ack exists.
    """

    def test_undeclared_statuses_keep_the_builtin_set(self):
        assert _declared_retry_statuses(None) == {429, 500, 503, 504}
        error_map = _map({"http": {"418": "config"}})
        assert _declared_retry_statuses(error_map) == {429, 500, 503, 504}

    def test_declared_fatal_status_leaves_the_retry_set(self):
        error_map = _map({"http": {"503": "auth"}})
        statuses = _declared_retry_statuses(error_map)
        assert 503 not in statuses
        assert {429, 500, 504}.issubset(statuses)

    def test_declared_retryable_status_joins_the_retry_set(self):
        error_map = _map({"http": {"402": "rate_limited"}})
    ...
    async def test_connect_applies_the_declared_policy(self):
        ...
        runtime = MagicMock()
        runtime.connector_id = "demo"
        runtime.declared_error_map = {"http": {"503": "config", "402": "transient"}}
        runtime.materialize = AsyncMock()
        runtime.raw_config = {}
        runtime.session.headers = {}
        await handler.connect(runtime)
        assert 503 not in handler._retry_statuses
        assert 402 in handler._retry_statuses
