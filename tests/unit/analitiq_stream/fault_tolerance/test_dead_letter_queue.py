...
class TestDeadLetterQueue:
    ...
    async def test_send_to_dlq_basic(self):
        ...
        with open(dlq_files[0], encoding="utf-8") as f:
    ...
    async def test_send_to_dlq_with_context(self):
        ...
        additional_context = {"stage": "transform", "batch_id": 7, "retry_attempt": 2}

        await dlq.send_to_dlq(
            record, error, pipeline_id, additional_context=additional_context
        )
    ...
    async def test_send_batch_to_dlq(self):
        ...
        await dlq.send_batch(
            batch, error_message, pipeline_id, additional_context=context
        )
    ...
    async def test_multiple_records_same_file(self):
        ...
        dlq = DeadLetterQueue(
            dlq_path=str(self.dlq_path), max_file_size=1024 * 1024
        )  # Large limit
    ...
    async def test_get_failed_records_filtered_by_stream(self):
        ...
        await dlq.send_to_dlq(
            {"id": 1}, Exception("e"), "pipeline1", stream_id="stream-a"
        )
        await dlq.send_to_dlq(
            {"id": 2}, Exception("e"), "pipeline1", stream_id="stream-b"
        )
        await dlq.send_to_dlq(
            {"id": 3}, Exception("e"), "pipeline2", stream_id="stream-a"
        )
...
class TestDeadLetterQueueEdgeCases:
    ...
    async def test_write_to_dlq_file_error_fallback(self):
        ...
        with open(fallback_files[0], encoding="utf-8") as f:
    ...
    async def test_write_to_dlq_fallback_also_fails(self):
        """A CRITICAL log is emitted and no exception propagates.

        Applies when both the primary and fallback writes fail.
        """
        dlq = DeadLetterQueue(dlq_path=str(self.dlq_path))
    ...
    async def test_send_batch_emits_actual_written_count(self):
        ...
        with patch("builtins.open", side_effect=OSError("disk full")):
            with patch("src.state.dead_letter_queue.emit_dlq_log") as mock_emit:
                await dlq.send_batch([{"id": 1}, {"id": 2}], "boom", "test-pipeline")
    ...
    async def test_encoder_handles_engine_value_types(self):
        ...
        written = await dlq.send_to_dlq(
            record, Exception("cast failed"), "test-pipeline"
        )
        assert written is True
        ...
        assert original["created"] == "2026-01-02T03:04:05+00:00"
    ...
    async def test_traceback_extraction_failure(self):
        ...
        with patch(
            "traceback.format_exception", side_effect=Exception("Traceback error")
        ):
    ...
    async def test_cleanup_old_files_error_handling(self):
        ...
        with patch("pathlib.Path.glob", side_effect=Exception("Glob error")):
            with patch("src.state.dead_letter_queue.logger") as mock_logger:
                ...
                mock_logger.error.assert_called_once()
                assert (
                    "Failed to cleanup old DLQ files"
    ...
    async def test_large_record_handling(self):
        ...
        dlq = DeadLetterQueue(
            dlq_path=str(self.dlq_path), max_file_size=1024
        )  # Small limit
        ...
        await dlq.send_to_dlq(
            large_record, Exception("Large record error"), "test-pipeline"
        )
