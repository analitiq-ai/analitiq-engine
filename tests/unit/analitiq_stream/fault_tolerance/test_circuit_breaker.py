...
class TestDatabaseCircuitBreaker:
    ...
    def test_database_circuit_breaker_initialization(self):
        ...
        assert db_cb.failure_threshold == 5  # Database-specific default
        assert db_cb.recovery_timeout == 30  # Shorter timeout for DB
        assert db_cb.expected_exception == Exception

    def test_database_circuit_breaker_custom_config(self):
        """Test database circuit breaker with custom config."""
        db_cb = DatabaseCircuitBreaker(
            # DatabaseCircuitBreaker doesn't accept params
        )

        # Default values from DatabaseCircuitBreaker
...
class TestAPICircuitBreaker:
    ...
    def test_api_circuit_breaker_initialization(self):
        ...
        assert api_cb.failure_threshold == 3  # API-specific default
        assert api_cb.recovery_timeout == 60  # Standard timeout for API
        # API circuit breaker has multiple expected exceptions

    def test_api_circuit_breaker_custom_config(self):
        """Test API circuit breaker with custom config."""
        api_cb = APICircuitBreaker(
            # APICircuitBreaker doesn't accept params
        )

        assert api_cb.failure_threshold == 3
        assert api_cb.recovery_timeout == 60
    ...
    async def test_api_circuit_breaker_http_errors(self):
        ...
        async def api_timeout_error():
            # ClientTimeout is a dataclass, not a BaseException, so raising it
            # surfaces a TypeError; the breaker still counts it as a failure.
        ...
        with pytest.raises(TypeError, match="derive from BaseException"):
            await api_cb.call(api_timeout_error)

        with pytest.raises(aiohttp.ClientConnectionError, match="Connection failed"):
            await api_cb.call(api_connection_error)

        with pytest.raises(TypeError, match="derive from BaseException"):
            await api_cb.call(api_timeout_error)

        assert api_cb.failure_count == 3
...
class TestCircuitBreakerEdgeCases:
    ...
    def test_circuit_breaker_invalid_configuration(self):
        ...
        # success_threshold parameter doesn't exist in this implementation
