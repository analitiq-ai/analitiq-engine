...
import generate_docker_requirements as gdr
import pytest
...
class TestSortKey:
    def test_extracts_the_bare_package_name(self) -> None:
        assert gdr._sort_key('aiohttp==3.14.3 ; python_version >= "3.11"') == "aiohttp"

    def test_is_case_insensitive(self) -> None:
        assert gdr._sort_key("PyYAML==6.0.3") == "pyyaml"

    def test_a_name_that_is_a_prefix_of_another_sorts_first(self) -> None:
    ...
    def test_rejects_a_line_with_no_leading_package_name(self) -> None:
...
class TestMain:
    ...
    def test_write_creates_the_file_with_rendered_content(
        self, monkeypatch: pytest.MonkeyPatch, _requirements_path: Path
    ) -> None:
        monkeypatch.setattr(subprocess, "run", _fake_run_success)
