...
class TestBaseDatabaseDriver:
    ...
    def test_acquire_connection_method_exists(self):
        """Test that acquire_connection is defined on the base class."""
        assert hasattr(BaseDatabaseDriver, 'acquire_connection')

    def test_engine_attribute_exists(self):
        """Test that _engine attribute is initialized in __init__."""
        driver = MockDatabaseDriver()
        assert hasattr(driver, '_engine')
        assert driver._engine is None
    ...
    @pytest.mark.asyncio
    async def test_acquire_connection_with_engine(self, driver):
        """Test acquire_connection when engine exists."""
        await driver.create_connection_pool({})

        async with driver.acquire_connection() as conn:
            # Should yield a connection from the engine
            assert conn is not None

    @pytest.mark.asyncio
    async def test_acquire_connection_without_engine(self, driver):
        """Test acquire_connection when engine doesn't exist."""
