"""Unit tests for the per-stream cursor checkpoint store and value encoding.
...
branches -- you can't easily torn-write a checkpoint mid-Docker-run. These tests
lock those branches: a tagged cursor value must round-trip with its type intact,
and any unreadable or unwritable checkpoint must degrade to a logged full
re-scan rather than crash the run.
"""
...
def test_decimal_cursor_round_trips_as_decimal(tmp_path):
    # A decimal must come back a Decimal (not a bare string), else the source
    # would mis-bind a NUMERIC/DECIMAL cursor column.
    store = CursorStore(tmp_path)
