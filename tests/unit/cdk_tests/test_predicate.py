...
class TestComparisons:
    ...
    def test_operator_truth_table(self, operator, left, right, expected):
    ...
    def test_eq_against_a_boolean_body_field(self):
    ...
    def test_unresolved_comparison_operand_raises(self):
    ...
    def test_unorderable_operands_raise(self):
...
class TestAbsenceOperators:
    def test_exists_and_missing_are_complementary(self):
        ...
        assert not evaluate_predicate(
            {"missing": {"ref": "response.body.next"}}, resolver
        )
    ...
    def test_empty_values_are_empty(self, value):
    ...
    def test_zero_and_false_are_not_empty(self, value):
        ...
        assert not evaluate_predicate(
            {"empty": {"ref": "response.body.total"}}, resolver
        )
        assert evaluate_predicate(
            {"not_empty": {"ref": "response.body.total"}}, resolver
        )

    def test_not_empty_on_a_populated_list(self):
        resolver = _resolver(body={"objects": [{"id": 1}]})
        assert evaluate_predicate(
            {"not_empty": {"ref": "response.body.objects"}}, resolver
        )
...
class TestNesting:
    ...
    def test_and_is_false_when_one_branch_fails(self):
...
class TestAuthoringDefects:
    def test_no_operator_key_raises(self):
        with pytest.raises(TransportSpecError, match="exactly one operator"):
            evaluate_predicate({}, _resolver())

    def test_two_operator_keys_raise(self):
        node = {
    ...
    def test_extension_keys_are_not_operators(self):
    ...
    def test_non_object_predicate_raises(self):
        with pytest.raises(TransportSpecError, match="must be an object"):
            evaluate_predicate("stop", _resolver())

    def test_unknown_scope_is_not_swallowed_as_missing(self):
