...
-- e2e-bigquery-to-postgres pipeline. Replace <dataset> with the dataset
-- configured on that connection (same value as dataset_id).
--
--   bq query --use_legacy_sql=false < tests/e2e_cloud_seed/bigquery_seed.sql

CREATE OR REPLACE TABLE `<dataset>.e2e_seed` (
    id         INT64    NOT NULL,
...
INSERT INTO `<dataset>.e2e_seed` (id, name, email, score, created_at, updated_at) VALUES
