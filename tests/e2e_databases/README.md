...
#    state/resume.json (the consolidated bookmark) and the per-stream checkpoint.
(cd docker && PIPELINE_ID=$PIPELINE_ID docker compose run --rm source_engine)
cat state/resume.json   # -> {"<stream_id>": 5}
...
# 3. (optional, proves the cloud path) delete the per-stream checkpoint but keep
#    state/resume.json, so resume.json is the ONLY bookmark left -- exactly the
#    fresh-container case where the deployment delivers resume.json in the bundle.
rm -rf state/$PIPELINE_ID

# 4. run 2. It restores the cursor from state/resume.json ("restored durable
#    cursor state ... from state/resume.json") and reads only ids 5,6,7 (the
#    inclusive >= boundary), not the whole table.
(cd docker && PIPELINE_ID=$PIPELINE_ID docker compose run --rm source_engine)
...
`state/resume.json` advances to `{"<stream_id>": 7}`.
