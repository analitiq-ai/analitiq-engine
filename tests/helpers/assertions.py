...
def assert_timestamp_fields_are_datetime(record: dict[str, Any], fields: list[str]):
    ...
    for field in fields:
        if field in record and record[field] is not None:
            assert isinstance(
                record[field], datetime
            ), f"Field {field} should be datetime, got {type(record[field])}"
...
def assert_timestamp_fields_are_strings(record: dict[str, Any], fields: list[str]):
    ...
    for field in fields:
        if field in record and record[field] is not None:
            assert isinstance(
                record[field], str
            ), f"Field {field} should be string, got {type(record[field])}"
            assert record[field].endswith("+00:00") or record[field].endswith(
                "Z"
            ), f"Field {field} should be ISO format with timezone"
...
def assert_database_error_contains_type_mismatch(error_msg: str):
    ...
    assert (
        "expected a datetime.date or datetime.datetime instance, got 'str'" in error_msg
    )
    assert "invalid input for query argument" in error_msg


def assert_record_has_expected_structure(
...
def assert_pipeline_config_valid(config: dict[str, Any]):
    ...
    assert (
        "connection_id" in config["destination"]
    ), "destination config missing connection_id"


def assert_batch_data_types_match_schema(
    batch: list[dict[str, Any]], schema_fields: dict[str, str]
):
        ...
        for field, expected_type in schema_fields.items():
            if field in record and record[field] is not None:
                ...
                if expected_type == "datetime":
                    assert isinstance(
                        record[field], datetime
                    ), f"Field {field} should be datetime, got {actual_type}"
                elif expected_type == "str":
                    assert isinstance(
                        record[field], str
                    ), f"Field {field} should be string, got {actual_type}"
                elif expected_type == "int":
                    assert isinstance(
                        record[field], int
                    ), f"Field {field} should be integer, got {actual_type}"
                elif expected_type == "float":
                    assert isinstance(
                        record[field], (int, float)
                    ), f"Field {field} should be numeric, got {actual_type}"
