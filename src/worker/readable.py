                ...
                async for response in stub.ReadStream(request):
                    ...
                    if kind == "batch":
                        ...
                        if err.deterministic:
                            ...
                            # deterministic source error classifies as
                            # CONFIG_INVALID (or SCHEMA_MISMATCH for a type-map
                            # miss) regardless of the collapsed wrapper's class
                            # name; the engine otherwise only sees ReadError text.
                            # The original connector class survives in the
                            # error_type prefix, so the source classifier reads it.
