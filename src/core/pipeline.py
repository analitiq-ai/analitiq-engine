...
from ..fault_tolerance.log_storage import LogStorageSettings, create_log_handler
...
from ..config.connector_sync import ConnectorSync

logger = logging.getLogger(__name__)


def _sync_connectors_if_configured() -> None:
    """
    Sync connectors from GitHub if analitiq.yaml is configured.

    Called automatically at Pipeline initialization.
    Silently skips if no config file is found.
    """
    try:
        ...
        for name, status in results.items():
            if "error" in status.lower():
                logger.warning(f"Connector sync warning: {name}: {status}")
            else:
                logger.info(f"Connector: {name}: {status}")

    except FileNotFoundError:
        # No analitiq.yaml found - skip silently
        logger.debug("No analitiq.yaml found, skipping connector sync")
    except Exception as e:
        # Log warning but don't fail pipeline initialization
        logger.warning(f"Connector sync failed: {e}")
...
class Pipeline:
    ...
    def __init__(
        ...
        # Sync connectors from GitHub if analitiq.yaml is configured
        _sync_connectors_if_configured()
        ...
        # Build engine config from runtime settings
        ...
        engine_config = {
            ...
            "buffer_size": runtime.engine.buffer_size,
        }
        ...
        for stream_id, stream_data in streams.items():
            ...
            source = SourceConfig(
                ...
                replication=ReplicationConfig(
                    method=source_data.get("replication_method", "incremental"),
    ...
    def _setup_pipeline_logging(self, pipeline_id: str):
        ...
        self._log_settings.local_logs_dir = self.logs_dir
        ...
        root_package_logger = logging.getLogger("analitic-code")
    ...
    def _build_config_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "streams": streams,
            "runtime": self.pipeline_config.runtime.model_dump(),
            "monitoring": {
                "log_level": self.pipeline_config.runtime.logging.log_level,
                "metrics_enabled": self.pipeline_config.runtime.logging.metrics_enabled,
    ...
    async def run(self):
        ...
        progress_monitoring = self.pipeline_config.runtime.logging.progress_monitoring == "enabled"
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        for stream in self.stream_configs:
            dest = stream.get_primary_destination()
            streams_info[stream.stream_id] = {
                "name": stream.stream_id,
                "source": stream.source.endpoint_id,
                "destination": dest.endpoint_id,
        ...
        return {
            "pipeline_id": self.pipeline_config.pipeline_id,
            "name": self.pipeline_config.name,
            "enabled": self.pipeline_config.is_active,
