...
class PipelineMetrics(BaseModel):
    ...
    records_processed: int = Field(
        default=0, ge=0, description="Total number of records successfully processed"
    )
    records_failed: int = Field(
        default=0, ge=0, description="Total number of records that failed processing"
    )
    ...
    batches_processed: int = Field(
        default=0, ge=0, description="Total number of batches successfully processed"
    )
    batches_failed: int = Field(
        default=0, ge=0, description="Total number of batches that failed processing"
    )
    streams_processed: int = Field(
        default=0, ge=0, description="Total number of streams successfully completed"
    )
    streams_failed: int = Field(
        default=0, ge=0, description="Total number of streams that failed"
    )
    ...
    def has_failures(self) -> bool:
        ...
        return (
            ...
            or self.batches_failed > 0
            or self.streams_failed > 0
        )
