...
class APIConnector(BaseConnector):
                    ...
                    if batch:
                        ...
                        if cursor_field and cursor_field in last_record:
                            ...
                            primary_cursor = CursorField(
                                field=cursor_field,
                                value=cursor_value,
                                inclusive=state.get("cursor_mode") == "inclusive"
                            )
    ...
    def _load_state_from_sharded_manager(
        ...
        return {
            ...
            "safety_window_seconds": config.get("safety_window_seconds", 120)
        }
    ...
    async def _setup_incremental_replication(self, config: Dict[str, Any], state: Dict[str, Any], src_config: Dict[str, Any]):
        ...
        safety_window = state["safety_window_seconds"]
        effective_start = self._compute_effective_start_time(cursor, safety_window)
        
        # Build incremental filter using the mapped API parameter
        cursor_mode = state["cursor_mode"]
        incremental_filter = self._build_replication_filter(filter_param, cursor_mode, effective_start)
        ...
        logger.info(f"Set up incremental replication: {cursor_field} -> {filter_param} from {effective_start} (mode: {cursor_mode})")
    ...
    def _build_replication_filter(self, filter_param: str, cursor_mode: str, effective_start: str) -> Dict[str, Any]:
        """Build the filter parameters for incremental replication."""
        filter_params = {}
        filter_params[filter_param] = effective_start
        
        logger.debug(f"Built incremental filter: {filter_param} = {effective_start} (mode: {cursor_mode})")
    ...
    def _deduplicate_records(self, batch: List[Dict[str, Any]], state: Dict[str, Any], config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for record in batch:
            if self._is_record_new(record, bookmark, cursor_field, tie_breaker_fields, state.get("cursor_mode", "inclusive")):
    ...
    def _is_record_new(self, record: Dict[str, Any], bookmark: Dict[str, Any], cursor_field: str, tie_breaker_fields: List[str], cursor_mode: str) -> bool:
        ...
        try:
            ...
            if record_cursor_dt > stored_cursor_dt:
                ...
                # Same cursor value, check tie-breakers
                return self._compare_tie_breakers(record, tie_breaker_fields, bookmark, cursor_mode == "inclusive")
                
        except Exception as e:
            logger.warning(f"Error comparing record cursor: {str(e)}, treating as new")
            return True

    def _compare_tie_breakers(self, record: Dict[str, Any], tie_breaker_fields: List[str], bookmark: Dict[str, Any], inclusive: bool) -> bool:
        """Compare tie-breaker field values to determine if record is new."""
        if not tie_breaker_fields:
            return not inclusive  # If no tie-breakers and inclusive mode, treat as duplicate
        ...
        if "tiebreakers" in aux_data:
            ...
            # All tie-breaker fields are equal
            return not inclusive  # In inclusive mode, equal means duplicate
            
        # No tie-breaker information found
        return not inclusive  # In inclusive mode, if no tie-breaker, treat same cursor as duplicate
...
class RateLimiter:
    """Rate limiter for API requests with validation."""
